...
// Navigate back if no active table
    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      >
        ...
        <div class="p-6 border-b border-slate-200 bg-indigo-50 shrink-0">
          <div class="flex justify-between items-start mb-2">
            ...
            <div class="flex flex-col items-end">
              <span
                class="bg-indigo-100 text-indigo-800 text-xs font-bold px-2 py-1 rounded-md mb-1"
                >{{ activeTable.pax || 0 }} Pax</span
              >
              <span class="text-xs font-medium text-slate-400">{{
                store.tiers.find((t) => t.id === activeTable.tierId)?.name ||
                "No Tier"
              }}</span>
            </div>
          </div>
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-6 hidden-scrollbar">
          ...
          <div v-if="activeTable.orders && activeTable.orders.length > 0">
            <h4
              class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3"
            >
              {{ t("posModule.sentOrders") }}
            </h4>
            <div class="space-y-3">
              <div
              ...
              >
                <div class="flex items-center">
                  <span class="w-6 text-slate-500 font-medium"
                    >{{ item.qty }}x</span
                  >
                  <span class="text-slate-700 font-medium truncate w-40">{{
                    item.name
                  }}</span>
                </div>
            ...
            >
              {{ t("posModule.subtotalItems") }} ฿{{ existingOrderTotal }}
            </div>
          </div>
